...
PARAM_COSTOS_DEFAULT = {
    ...
    "directos": {
        ...
        "eficiencia":               75,  # % eficiencia productiva de las máquinas
    },
            ...
            if analizar_btn:
                    ...
                    with st.spinner(spinner_msg):
                        try:
                            ...
                            # Usar bytes del uploader actual O los guardados en session_state
                            if plano_file is not None:
                                file_bytes    = plano_file.read()
                                _nombre_plano = plano_file.name
                            else:
                                # Intentar desde session_state primero
                                file_bytes    = st.session_state.get(f"_plano_bytes_{pieza['id']}", None)
                                _nombre_plano = st.session_state.get(f"_plano_nombre_{pieza['id']}",
                                               pieza.get("plano_nombre", ""))
                                # Si no hay bytes, descargar desde Cloudinary
                                if not file_bytes:
                                    _plano_url = pieza.get("plano_url", "") or pieza.get("plano_drive_id", "")
                                    if _plano_url and _plano_url.startswith("http"):
                                        try:
                                            import requests as _req
                                            _r = _req.get(_plano_url, timeout=30)
                                            if _r.status_code == 200:
                                                file_bytes = _r.content
                                                st.session_state[f"_plano_bytes_{pieza['id']}"] = file_bytes
                                        except Exception as _e:
                                            pass
                            if not file_bytes:
                                st.error("❌ No hay plano disponible. Sube el plano primero.")
                                st.stop()
                            file_b64   = base64.b64encode(file_bytes).decode("utf-8")
                            is_pdf     = _nombre_plano.lower().endswith(".pdf")
...
if tab4 is not None:
    with tab4:
        ...
        with dc3:
            ...
            d["eficiencia"]   = st.number_input("Eficiencia productiva (%)",
                value=int(d.get("eficiencia", 75)), step=5, min_value=50, max_value=100,
                key="d_efic",
